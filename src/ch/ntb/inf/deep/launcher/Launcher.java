		...
		try {
			...
			while(item != null && reporter.nofErrors <= 0) {
				if(item instanceof Class){
					...
					if (reporter.nofErrors <= 0) Linker.calculateRequiredSize(clazz);
				}
				item = item.next;
			}

			// 4) Linker: freeze memory map
			if (reporter.nofErrors <= 0) Linker.freezeMemoryMap();
			...
			if (reporter.nofErrors > 0) {
				System.out.println("Compilation failed with "+reporter.nofErrors+" error(s)");
			}else{
				System.out.println("Compilation and target image generation successfully finished");
			}
		} catch (IOException e) {
