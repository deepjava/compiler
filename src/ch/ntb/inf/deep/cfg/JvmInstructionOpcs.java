...
interface JvmInstructionOpcs {
	...
	byte  // attributes of JVM instructions (byte code attribute position in look-up table)
		bcapBranch = 27,   bcapCondBranch = 26,   
		bcapUncondBranch = 25,   bcapReturn = 24, 
		bcapSwitch = 23, bcapCall = 22, 
		bcapNew = 21, bcapUndef = 20,
		bcapCFGnotImpl = 19,	bcapSSAnotImpl = 18,
		bcapCGnotImpl = 17;

	int  // bit masks of JVM instructions (byte code mask position in look-up table)
		bcmpOpcLen = 0xF00;
		
	int[] bcAttrTab = {
	/*Format:	0xsFFF'uLcc,
			s	change of operand stack pointer (in slots):		slotPointer := slotPointer + SignExtend(s)
				   S=-8: stack change depends on operand type
			F	Flags: {Branch, CondBranch, UncondBranch, Return, Switch, Call, New, } (see const declarations)
			u	unused
			L	instruction length: number of bytes (0 - any length)
		...
		*/
			...
			0x10000200 + bCfload, // push float from ..
			0x20000200 + bCdload, // push double from ..
			0x10000200 + bCaload, // push reference from ..
			...
			0xF0000200 + bCistore, // locVar[imm] = (int) top, pop
			0xE0000200 + bClstore, // locVar[imm,+1] = (long) top2, pop2
			0xF0000200 + bCfstore, // locVar[imm] = (float) top, pop
			0xE0000200 + bCdstore, // locVar[imm,+1] = (double) top2, pop2
			0xF0000200 + bCastore, // locVar[imm] = (reference) top, pop
			...
			0xF0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCifeq, // branch if ([top] == 0)
			0xF0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCifne, // branch if ([top] != 0)
			0xF0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCiflt, // branch if ([top] < 0)
			0xF0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCifge, // branch if ([top] >= 0)
			0xF0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCifgt,  // branch if ([top] > 0)
			0xF0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCifle,  // branch if ([top] <= 0)

			0xE0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCif_icmpeq, // branch if ([top-1] == [top] )
			0xE0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCif_icmpne, // branch if ([top-1] != [top] )
			0xE0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCif_icmplt, // branch if ([top-1] < [top] )
			0xE0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCif_icmpge, // branch if ([top-1] >= [top] )
			0xE0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCif_icmpgt, // branch if ([top-1] > [top] )
			0xE0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCif_icmple, // branch if ([top-1] <= [top] )

			0xE0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCif_acmpeq, // branch if ([top-1] == [top] )
			0xE0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCif_acmpne, // branch if ([top-1] != [top] )
		
			0x00000300 + (1<<bcapUncondBranch) | (1<<bcapBranch) | bCgoto, // branch

			0x10000300 + (1<<bcapCall) | (1<<bcapCFGnotImpl) | bCjsr, // jump to subroutine

			0x00000200 + (1<<bcapReturn) | (1<<bcapBranch) | (1<<bcapCFGnotImpl) | bCret, // return ref		

			0xF0000000 + (1<<bcapSwitch) | (1<<bcapBranch) | bCtableswitch,
			0xF0000000 + (1<<bcapSwitch) | (1<<bcapBranch) | bClookupswitch,

			0xF0000100 + (1<<bcapReturn) | bCireturn, // return int
			0xE0000100 + (1<<bcapReturn) | bClreturn, // return long
			0xF0000100 + (1<<bcapReturn) | bCfreturn, // return float
			0xE0000100 + (1<<bcapReturn) | bCdreturn, // return double
			...
			0x80000400 + (1<<bcapCall) | (1<<bcapNew) | bCmultianewarray, // create new multidimensional array

			0xF0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCifnull, // branch if ([top] == null)
			0xF0000300 + (1<<bcapCondBranch) | (1<<bcapBranch) | bCifnonnull, // branch if ([top] != null)

			0x00000500 + (1<<bcapUncondBranch) | (1<<bcapBranch) |(1<<bcapCFGnotImpl) |  bCgoto_w,
			0x10000500 + (1<<bcapCall) |(1<<bcapCFGnotImpl) |  bCjsr_w,
